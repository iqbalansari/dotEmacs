...
* Check if project is indexed
  #+begin_src emacs-lisp
    ...
    (when iqbal-rtags-available
      (let* ((project-root (or (ignore-errors (projectile-project-root)) default-directory))
        ...
        (if project-indexed
          ...
          (let ((compile-commands (or (iqbal-locate-compile-commands default-directory)
                                      (iqbal-locate-compile-commands project-root))))
            (if compile-commands
