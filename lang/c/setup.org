...
* Load malinka
  #+begin_src emacs-lisp
    (when (and (not (boundp 'malinka-projects-map))
      ...
      (load "malinka")
      (when (file-exists-p (concat my-home-dir "malinka-projects.el"))
        (load (concat my-home-dir "malinka-projects.el"))))
  #+end_src
...
** Select whichever checker is available
   #+begin_src emacs-lisp
     (cond ((not (version< my-gcc-version "4.8.1")) (flycheck-select-checker 'c/c++-gcc))
           (my-clang-version (flycheck-select-checker 'c/c++-clang)))
   #+end_src
