...
* Do not prompt to apply local vars if user has said no once
  #+begin_src emacs-lisp
    (defvar iqbal-local-vars-file "~/.emacs.d/.disabled-local-vars"
      "Directories with ignore local vars are stored here")
    
    (defvar iqbal-local-vars-ignored (when (file-exists-p iqbal-local-vars-file)
    ...
    restored on startup"
      (iqbal-write-to-file iqbal-local-vars-file
                           (prin1-to-string iqbal-local-vars-ignored)))
    ...
    (defun iqbal-enable-local-vars-for (dir)
      ...
      (interactive (list (ido-completing-read "Enable local vars for: "
                                              iqbal-local-vars-ignored)))
      (setq iqbal-local-vars-ignored (delete dir iqbal-local-vars-ignored)))
    ...
    remember the response if it is negative"
      (unless (memq dir-name iqbal-local-vars-ignored)
        ad-do-it
        (unless ad-return-value
          (add-to-list 'iqbal-local-vars-ignored dir-name)))
