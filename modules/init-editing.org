...
* Enable cua-selection-mode
  #+begin_src emacs-lisp
    ...
    (set-face-attribute 'cua-rectangle nil :inherit 'region :background 'unspecified :foreground 'unspecified)
  #+end_src
...
* Copy/cut current line if no region is selected, if rectangle is selected use it
  #+begin_src emacs-lisp
    ...
    (defun cut-line-or-region (arg)
      ...
      (interactive "p")
      (cond (cua--rectangle (progn (call-interactively 'cua-cut-rectangle)
