* Install org-projectile
  ...
  #+begin_src emacs-lisp
    (my-install-packages '(org-projectile org-journal))
  #+end_src
...
*** Get active region in previous buffer as quote and org link to it
    #+begin_src emacs-lisp
      ...
      (defun my-get-source-buffers-region-and-link ()
        (save-window-excursion
          (switch-to-prev-buffer)
          (let ((fill-prefix "    "))
            (my-indent-text (concat (when (region-active-p)
                                      (concat "#+begin_quote\n"
                                              (my-indent-text (my-justify-paragraph-text (buffer-substring (region-beginning)
                                                                                                           (region-end)))
                                                              2)
                                              "\n#+end_quote\n"))
                                    (org-store-link t))
...
** Global keybindings for org-mode
  #+begin_src emacs-lisp
    ...
    (global-set-key (kbd "<f8>") #'org-tags-view)
