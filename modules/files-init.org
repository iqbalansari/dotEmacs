    ...
    visiting"
      ...
      (cond 
       (current-prefix-arg (setq kill-text (buffer-name))
                           (setq what " buffer's "))
       ((eq major-mode 'dired-mode) (setq kill-text (expand-file-name default-directory)) (setq what " directory's "))
       ((buffer-file-name) (setq kill-text (buffer-file-name)) (setq what " file's "))
       (t (setq kill-text nil)
          (message "Nothing to copy")))
      (when kill-text
        (kill-new kill-text)
        (message (concat "Copied" what "path"))))
