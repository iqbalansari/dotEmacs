...
** 6. Combine the code in the given org-files to a single string
   #+begin_src emacs-lisp
     (defun my-combine-org-files (target sources language)
       (let* ((extracted-files (mapcar (lambda (source)
              ...
              (extracted-code (reduce (lambda (code input-file)
                                        (if input-file
                                            (concat code (my-read-file input-file))
                                          code))
                                      extracted-files :initial-value "")))
         (when target
           (my-write-to-file target extracted-code))
         extracted-code))
   #+end_src
...
** 7. Function to load language configuration
   ...
   #+begin_src emacs-lisp
     (defun my-load-lang (language)
       ...
       (let* ((lang-path (concat my-langs-dir language "/"))
         ...
         (when (or (not (file-exists-p compiled-file))
                   (apply #'my-is-file-stale compiled-file source-files))
           (my-combine-org-files compiled-file source-files "emacs-lisp")
