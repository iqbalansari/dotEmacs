...
** 1. Path to packages not in a package manager
   ...
   #+begin_src emacs-lisp
     ...
     (dolist (package-dir (remove-if-not #'file-directory-p (directory-files otherpackages t "^[^.]+")))
...
** Setup installation of external language modes
  #+begin_src emacs-lisp
    ...
    (my-auto-install-lang (rx word-start (zero-or-one ".") "ledger" string-end) 'ledger-mode 'ledger-mode)
  #+end_src
