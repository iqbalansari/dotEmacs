          ...
          (when (bongo-buffer-p)
            ...
            (when bongo-player
              (let* ((elapsed-time (bongo-player-elapsed-time bongo-player))
                     (stop-after (when elapsed-time
                                   (read-number "[ Link to currently playing media ] Number of seconds after which to stop playback, if 0 stop when the media ends: " 0)))
                     (stop-time (when (and stop-after (not (zerop stop-after)))
                                  (+ elapsed-time stop-after)))
                     (file-name (bongo-player-file-name bongo-player))
                     (description (iqbal-get-player-state-org-description bongo-player elapsed-time stop-time))
                     (link (concat "bongo:"
                                   (url-hexify-string file-name)
                                   (when elapsed-time
                                     (concat (format "::%d" elapsed-time)
                                             (when stop-time
                                               (format "-%d" stop-time)))))))
