...
** Additional keys on search map
   #+BEGIN_SRC emacs-lisp
     ...
     (define-key search-map "m" #'multi-occur-in-this-mode)
     ...
     (define-key search-map "c" #'lgrep)
     (define-key search-map "r" #'rgrep)
     (define-key search-map "z" #'zrgrep)
     (define-key search-map "p g" #'projectile-grep)
...
** Keybindings for ag
   #+BEGIN_SRC emacs-lisp
     (when (executable-find "ag")
       (define-key search-map "p s" #'projectile-ag)
       (define-key search-map "a s" #'ag)
       (define-key search-map "a f" #'ag-files)
       (define-key search-map "a d" #'ag-dired)
       (define-key search-map "a p" #'ag-dired))
   #+END_SRC
