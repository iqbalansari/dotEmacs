* Install packages useful for search and replace
  #+BEGIN_SRC emacs-lisp
    ...
    (when (executable-find "rg")
      (add-to-list 'iqbal-search-replace-packages 'rg))
...
** Keybindings for ag/rg
   #+BEGIN_SRC emacs-lisp
    (when (executable-find "ag")
       (define-key search-map (kbd "p a") #'projectile-ag)
       (define-key search-map (kbd "a s") #'ag)
       (define-key search-map (kbd "a f") #'ag-files)
       (define-key search-map (kbd "a d") #'ag-dired))

     (when (executable-find "rg")
       (define-key search-map (kbd "p r") #'projectile-ripgrep)
       (define-key search-map (kbd "a s") #'rg))
   #+END_SRC
