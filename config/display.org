...
* Use smooth-scrolling for better scrolling
  Dynamically reset scroll margin on window resize to prevent awkward
  jumps
  #+begin_src emacs-lisp
    (make-variable-buffer-local 'smooth-scroll-margin)

    (defun iqbal-set-smooth-scroll-margin (&optional frame)
    ...
    (add-hook 'window-size-change-functions #'iqbal-set-smooth-scroll-margin)
  #+end_src
