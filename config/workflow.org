* Install org-projectile, orgit and org-journal
  #+begin_src emacs-lisp
    (my-install-packages '(org-projectile org-journal orgbox orgit noflet))
    (require 'orgbox)
  #+end_src
...
*** Template for project specific TODOs
    #+begin_src emacs-lisp
      (defun my-enable-org-projectile ()
        (load "org-projectile")
        (add-to-list 'org-capture-templates 
                     (org-projectile:project-todo-entry "p" "* TODO %? :%(my-get-source-buffers-project):\n%a\n")))

      (eval-after-load "projectile" #'my-enable-org-projectile)
    #+end_src
...
* Change TODO state to READING when opening a UNREAD link
  #+begin_src emacs-lisp
    (defun my-org-mark-link-as-reading-on-follow ()
      (let ((todo-state (save-excursion (org-back-to-heading t)
                                        (org-get-todo-state))))
        (when (string= todo-state "UNREAD")
          (org-todo "READING"))))

    (add-hook 'org-follow-link-hook #'my-org-mark-link-as-reading-on-follow)
  #+end_src


* Clock in automatically if TODO state changes to STARTED or READING
  #+begin_src emacs-lisp
    (defun my-org-clock-in-on-todo-change ()
      (when (and org-state
    ...
    (add-hook 'org-after-todo-state-change-hook #'my-org-clock-in-on-todo-change)
  #+end_src
...
* Clock into a task interactively
  ...
  #+begin_src emacs-lisp
    (defun my-extract-todos (file matcher)
    ...
    (defun my-clock-in-interactive ()
      (interactive)
      (if (and (org-clocking-p)
               (not current-prefix-arg)
               (y-or-n-p "Are you sure you want to clock out?"))
          (progn (org-clock-out)
        ...
        (let* ((todo-only t)
               ...
               (org-clock-history (mapcar (lambda (todo) (get-text-property 0 'org-marker todo))
                                          (loop for agenda-file in (org-agenda-files)
                                                append (my-extract-todos agenda-file matcher)))))
          (org-clock-in '(4)))))
  #+end_src
                                       ...
                                       (if (org-clocking-p)
                                         ...
                                         (org-propertize "Not clocked in"
                                                         ...
                                                         'keymap '(mode-line keymap
                                                                             (mouse-1 . my-clock-in-interactive)
...
** Global keybindings for org-mode
  #+begin_src emacs-lisp
    ...
    (global-set-key (kbd "<f6>") #'my-clock-in-interactive)
    (global-set-key (kbd "<f8>") #'my-org-hide/show-current-task)
    (global-set-key (kbd "<f9>") #'org-tags-view)
    (global-set-key (kbd "<f10>") #'my-view-lagging-tasks)
