...
** 2. Path to 'lang'uage specific configurations
   Modules contain the configuration related to one specific type of file.
   They reside in the following directory
   #+begin_src emacs-lisp
     (defvar iqbal-modules-dir "~/.emacs.d/lang/")
   #+end_src

** 3. Function to load module
   A simple helper function to load a particular module
   #+begin_src emacs-lisp
     (defun iqbal-load-module (language)
	   (let ((module-path (concat iqbal-modules-dir language "/"))
